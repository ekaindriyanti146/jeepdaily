...
# Global Tracker untuk memastikan keunikan total gambar per sesi
SESSION_USED_IMAGE_IDS = set()

# ==========================================
# 📸 THE ULTIMATE JEEP & OFFROAD POOL
# ==========================================
# Semua ID di bawah ini dikurasi manual: HANYA JEEP atau GUNUNG/OFFROAD.
JEEP_MASTER_POOL = {
    "wrangler": [
        ...
        "1606820311337-3367f0b982f5", "1620300484797-2a45638a168b", "1591462319086-4f90113f9f3b"
    ],
    "classic": [
...
# ==========================================
# 🎨 UNSPLASH ENGINE (QUEUE SYSTEM - NO DUPES)
# ==========================================

def modify_image_to_be_unique(img):
    """Manipulasi gambar agar memiliki sidik jari pixel unik & Branded Watermark."""
    try:
        # Flip & Subtle Rotation
        ...
        # ✨ WATERMARK @JeepDaily (POSISI AMAN & RAPI) ✨
        ...
        # Margin 55px dari kanan & 50px dari atas
        text_x = 1200 - 120 - 55 
        text_y = 50
        draw_txt.text((text_x, text_y), watermark_text, fill=(255, 255, 255, 150), font=font)
...
def generate_unsplash_image(keyword, filename):
    ...
    pool_key = 'nature' # Default Fallback Relevan
    if any(x in keyword for x in ['wrangler', 'gladiator', 'rubicon', 'sahara', 'jeep']): 
        pool_key = 'wrangler'
    elif any(x in keyword for x in ['classic', 'willys', 'history', 'vintage', 'cj']): 
        pool_key = 'classic'
    
    # Ambil list dan acak urutannya
    current_pool = JEEP_MASTER_POOL[pool_key][:]
    ...
    while attempts < 5:
        # Cari ID yang belum dipakai sama sekali dalam sesi lari (run) ini
        available_ids = [i for i in current_pool if i not in SESSION_USED_IMAGE_IDS]
        if not available_ids: 
            SESSION_USED_IMAGE_IDS.clear() # Reset jika koleksi habis
            available_ids = current_pool
        ...
        try:
            print(f"      🎨 Downloading Unique {pool_key.upper()} ID: {selected_id}")
            ...
            if resp.status_code == 200:
                ...
                # VERIFIKASI FILE: Harus di atas 5KB (Anti-broken)
                if os.path.exists(output_path) and os.path.getsize(output_path) > 5120:
                    return f"/images/{filename}"
                else:
                    print(f"      ⚠️ Broken file detected for ID {selected_id}. Retrying...")
        except: pass
        attempts += 1
        time.sleep(1)
...
# ==========================================
# 🧠 ANTI-GENERIC CONTENT ENGINE (STRICT)
# ==========================================

def get_groq_article_json(title, summary, link, author_name):
    current_date = datetime.now().strftime("%Y-%m-%d")
    system_prompt = f"""
    You are {author_name}, a Jeep Expert. Write a high-quality 1000-word article.
    🛑 HARD RULE FOR HEADERS (H2, H3):
    - DO NOT use generic words like "Introduction", "Understanding", "Conclusion", "Overview", "Limitations".
    - Make every header specific to the Jeep model or off-road technique.
    - Example: Instead of "Introduction", use "The Wrangler Rubicon: A Rock-Crawling Beast".
    - Include a Markdown table for engine/technical specs.
    ...
    for source_name, rss_url in RSS_SOURCES.items():
        ...
        for entry in feed.entries:
            ...
            try:
                ...
                # Image Engine: Unique Per Session + Verified Relevance
